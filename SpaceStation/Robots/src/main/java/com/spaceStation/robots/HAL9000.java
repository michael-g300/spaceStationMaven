...
import com.spaceStation.robotTools.Disruptor;
import com.spaceStation.robotTools.LaserCutter;
import com.spaceStation.robotTools.Replicator;
...
public class HAL9000 implements Robot {
    private final String m_name;
    private final String m_callSign;
    private final List<Tool> m_tools;
    private RobotState m_state;
    private static final ReentrantLock lock = new ReentrantLock(false);
    public HAL9000(final String name, final String callSign) {
        m_name = name;
        m_callSign = callSign;
        m_tools = new ArrayList<>();
        m_tools.add(new LaserCutter());
        m_tools.add(new Replicator());
        m_tools.add(new Disruptor());

        final double successfulCreation = new SecureRandom().nextDouble(0, 1);
        m_state = successfulCreation <= 0.9 ? new ActiveState() : new FailingState();
    }
    @Override
    public void work() {
        try {
